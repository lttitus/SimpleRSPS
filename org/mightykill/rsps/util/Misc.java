...
package org.mightykill.rsps.util;
...
public class Misc {
    ...
    public static String decryptPlayerChat(ChatMessage pack, int totalChars) {
		try {
			if (totalChars == 0) {
				return "";
			}
			...
			String s = "";
			for (;;) {
				byte i_7_ = (byte) pack.nextUnsignedByte();
				if (i_7_ >= 0) {
					i_4_++;
				} else {
	...
	public static int getEmptySlot(Object[] array) {
		int index = 0;
		for(Object o:array) {
			if(o == null) {
				return index;
	...
	/**
     * Finds the highest number within a given set of numbers
     * @param numbers The numbers you want to test
	...
	public static int getRealLength(int[] array, int badObj) {
    	int count = 0;
    	for(Integer i:array) {
    		if(i != badObj) {
    			count++;
	...
	/**
	 * Gets the Players that are missing in b
	 * @param a Previous list (control)
	 * @param b New list	  (test)
	 * @return
	 */
	public static ArrayList<Player> getRemovingPlayers(ArrayList<Player> a, ArrayList<Player> b) {
		ArrayList<Player> diff = new ArrayList<Player>();
	...
	public static ArrayList<NPC> getRemovingNPCs(ArrayList<NPC> a, ArrayList<NPC> b) {
		ArrayList<NPC> diff = new ArrayList<NPC>();
	...
	/**
	 * Test if two Entities are within range of each other
	 * @param control The Entity we are looking around
	 * @param test The Entity we are looking for
	 * @return
	 */
	public static boolean withinDistance(Entity control, Entity test) {
